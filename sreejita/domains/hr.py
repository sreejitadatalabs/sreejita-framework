...
class HRDomain(BaseDomain):
    ...
    def validate_data(self, df: pd.DataFrame) -> bool:
        """
        HR data must have people signals, not revenue.
        """
        return any(
    ...
    def calculate_kpis(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if attrition and pd.api.types.is_numeric_dtype(df[attrition]):
            ...
            exits = status_series.str.contains("exit|left|terminated|resigned", na=False)
        ...
        if performance and pd.api.types.is_numeric_dtype(df[performance]):
            ...
            kpis["avg_performance_score"] = avg_perf
            kpis["low_performance_rate"] = (df[performance] < 3).mean()
    ...
    def generate_visuals(
        ...
        if employee and department:
            ...
            counts = df.groupby(department)[employee].nunique().sort_values(ascending=False)
        ...
        if salary and pd.api.types.is_numeric_dtype(df[salary]):
            ...
            plt.figure(figsize=(7, 4))
            df[salary].dropna().plot(kind="hist", bins=15, color="#ff7f0e")
        ...
        # -------- Visual 3: Performance Distribution --------
        if performance and pd.api.types.is_numeric_dtype(df[performance]):
            p = output_dir / "performance_distribution.png"

            plt.figure(figsize=(7, 4))
            df[performance].dropna().plot(kind="hist", bins=10, color="#2ca02c")
        ...
        # -------- Visual 4: Absence Analysis --------
        if absence and department and pd.api.types.is_numeric_dtype(df[absence]):
            p = output_dir / "absence_by_department.png"

            abs_by_dept = df.groupby(department)[absence].mean().sort_values(ascending=False)

            plt.figure(figsize=(7, 4))
            abs_by_dept.plot(kind="bar", color="#d62728")
            plt.title("Average Absence Days by Department")
            plt.xticks(rotation=45, ha="right")
            plt.tight_layout()
            plt.savefig(p)
            plt.close()
        ...
        if attrition is not None:
            if attrition > 0.20:
                insights.append({
                    ...
                    "so_what": f"Attrition rate is {attrition:.1%}, well above acceptable levels."
                })
            elif attrition > 0.10:
                insights.append({
                    "level": "WARNING",
                    "title": "Rising Attrition",
                    "so_what": f"Attrition rate is {attrition:.1%}, exceeding the 10% target."
                })
    ...
    def generate_recommendations(self, df: pd.DataFrame, kpis: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if attrition is not None and attrition > 0.20:
            recs.append({
